...
**CRITICAL: DO NOT COMMIT THE CODE**

- Your task is to fix the test and prove it passes
- DO NOT use git commit, git add, or any git operations that persist the fix
- Leave the changes unstaged/staged so they can be independently verified
- If you commit the code, you lose the ability to verify the original failure state
- The fix must be validated by running the test, not by hiding the broken state
