...
<If the bug affects UI, include a task to create a E2E test file. Your task should look like: "Read `.claude/e2e/test_basic_query.md` and `.claude/e2e/test_complex_query.md` and create a new E2E test file in `.claude/e2e/test_<descriptive_name>.md` that validates the bug is fixed, be specific with the steps to prove the bug is fixed. We want the minimal set of steps to validate the bug is fixed and screen shots to prove it if possible.">
...
- `uv run adws/adw_compile.py` - Compile code to validate the bug is fixed with zero regressions
